...
def test(list_signal,l):
    ...
    for i in list_signal:
        ...
        if i[2]==-2:
            ...
            profit_rate.append(totalprofit[-1]/1000000)
            if buy_lot[ID]==0:
                buy_price[ID]=0
        if i[2]==2:
            sell_lot[ID]=sell_lot[ID]-lot
...
##        print "wrong"
    ...
    if len(profit)==0:
        return 0
    if len(profit_rate)==0: print "wrong1"
    if mystd(profit_rate)==0: print profit_rate
    s=(sum(profit_rate)/len(profit_rate))/(mystd(profit_rate))
    if s<0:
        s=0
    return s
...
start=datetime.datetime(2011,1,14,0,0,0)
stop=datetime.datetime(2014,10,1,0,0,0)
mid=start+t1
end=start+t2+t1
row_test=[]
row_run=[]
